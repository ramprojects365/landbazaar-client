import { IBlogDT } from "@/types/blog-d-t";
import blogThumb1 from "../../public/assets/img/blog/10-essential-legal-documents-to-check-before-buying-a-plot-in-Hyderabad.png";
import blogThumb2 from "../../public/assets/img/blog/complete-guide-to-buying-managed-farmland-and-weekend-farmhouses.png";
import blogThumb3 from "../../public/assets/img/blog/verify-land-records-telangana-dharani-registration-portal.png";
import blogThumb4 from "../../public/assets/img/blog/hmda-vs-dtcp-vs-rera-whats-the-difference-and-which-is-safer.png";
import blogThumb7 from "../../public/assets/img/blog/why-west-hyderabad-shankarpally-to-sadashivpet-is-the-next-realty-hotspot.png";
import blogThumb8 from "../../public/assets/img/blog/top-5-high-growth-land-investment-corridors-around-hyderabad.png";
import blogThumb9 from "../../public/assets/img/blog/impact-of-the-regional-ring-road-rrr-on-hyderabad-farmland-prices.png";

export const blogData: IBlogDT[] = [
  {
    id: 1,
    image: blogThumb1,
    category: "Dekho Land",
    readTime: "5 min read",
    title:
      "10 Essential Legal Documents to Check Before Buying a Plot in Hyderabad",
    slug: "10-essential-legal-documents-to-check-before-buying-a-plot-in-Hyderabad",
    authorName: "Dekho Land",
    authorRole: "Land News",
    delay: ".3s",
  },
  {
    id: 2,
    image: blogThumb4,
    category: "Dekho Land",
    readTime: "4 min read",
    title: "HMDA vs. DTCP vs. RERA: What’s the Difference and Which is Safer?",
    slug: "hmda-vs-dtcp-vs-rera-whats-the-difference-and-which-is-safer",
    authorName: "Dekho Land",
    authorRole: "Investment News",
    delay: ".5s",
  },
  {
    id: 3,
    image: blogThumb3,
    category: "Dekho Land",
    readTime: "6 min read",
    title:
      "How to Verify Land Records on the Telangana Dharani / Registration Portal",
    slug: "verify-land-records-telangana-dharani-registration-portal",
    authorName: "Dekho Land",
    authorRole: "Land Guide",
    delay: ".7s",
  },
  {
    id: 4,
    image: blogThumb8,
    category: "Dekho Land",
    readTime: "7 min read",
    title: "Top 5 High-Growth Land Investment Corridors Around Hyderabad",
    slug: "top-5-high-growth-land-investment-corridors-around-hyderabad",
    authorName: "Dekho Land",
    authorRole: "Land Guide",
    delay: ".7s",
  },
  {
    id: 5,
    image: blogThumb9,
    category: "Dekho Land",
    readTime: "8 min read",
    title:
      "Impact of the Regional Ring Road (RRR) on Hyderabad Farm land Prices",
    slug: "impact-of-the-regional-ring-road-rrr-on-hyderabad-farmland-prices",
    authorName: "Dekho Land",
    authorRole: "Land Guide",
    delay: ".7s",
  },
  {
    id: 6,
    image: blogThumb7,
    category: "Dekho Land",
    readTime: "10 min read",
    title:
      "Why West Hyderabad (Shankarpally to Sadashivpet) is the Next Realty Hotspot",
    slug: "why-west-hyderabad-shankarpally-to-sadashivpet-is-the-next-realty-hotspot",
    authorName: "Dekho Land",
    authorRole: "Land Guide",
    delay: ".7s",
  },
  {
    id: 7,
    image: blogThumb2,
    category: "Dekho Land",
    readTime: "8 min read",
    title: "Complete Guide to Buying Managed Farmland and Weekend Farmhouses",
    slug: "complete-guide-to-buying-managed-farmland-and-weekend-farmhouses",
    authorName: "Dekho Land",
    authorRole: "Land Guide",
    delay: ".7s",
  },
];

export default blogData;
